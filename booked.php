...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
  ...
  <ul class="nav nav-tabs">
    <li class="nav-item">
      <a class="nav-link active" href="booked.php">Ordered</a>
    </li>
    <li class="nav-item">
  ...
  <h1 style="color: gray; margin:2rem">Order Check</h1>
  ...
  /*Inserting and displaying on date starts*/
  if (isset($_POST["datesubmit"])) {
    $date = $_POST["bookingdate"];

    $ssql = "SELECT * FROM `customerdetails` where date='$date' and status='ORDERED' ";
    ...
    echo "<div class='table-responsive' style=margin:2rem>
     <table class='table table-bordereless table-hover table-condensed small'>
       ...
       <th scope='col' style='color:rebeccapurple;'> DATE</th>
       <th scope='col' style='color:rebeccapurple;'> TIME</th>
    ...
    while ($row = mysqli_fetch_array($result)) {
      ...
      echo " <form name='orderfrm' action='insert.php' method='post'><div class='form-group'>
        ...
        <td scope='row'><input type='text' name='fullname' value=$row[1]_$row[2] style='color:darkslategray;font-weight:bold;' readonly/></td>
        <td scope='row'><input type='text' name='item' value=$row[3] style='color:mediumpurple;font-weight:bold'  readonly /></t>
        <td scope='row'><input type='number' name='quantity' value=$row[4]  style='color:tomato;font-weight:bold;' readonly/></td>
        <td scope='row'><input type='number' name='price' value=$row[5] style='color:tomato;font-weight:bold;' readonly/></td>
        <td scope='row'><input type='tel' name='phone' value=$row[6] style='color:mediumvioletred;font-weight:bold;' readonly/></td>
        <td scope='row'><input  type='email' name='email' value=$row[7]  style='color:darkslategrey;font-weight:bold' readonly/></td>
        <td scope='row'><input  type='text' name='address' value=$row[8]  style='color:mediumorchid;font-weight:bold' readonly/></td>
        ...
        <td scope='row'><button type='submit' class='btn btn-primary' name='dispatched'>Dispatch</button></td>";
      $tot = $tot + $row[4];
    }
    echo "<tr><th style='color:rebeccapurple;font-weight:bold' scope='row' colspan='13'>Number of order placed on " . $date . " is " . $nrow . "</th></tr>";
    echo "<tr><th style='color:rebeccapurple;font-weight:bold' scope='row' colspan='13'> Total Number of order(quantity) placed on " . $date . " is " . $tot . "</th></tr>";

    echo '</table></div>';

    /*Inserting and displaying on date ends*/
  }

  /*displaying booked data with code starts*/
  if (isset($_POST["codesubmit"])) {
    $code = $_POST["bookingcode"];

    $ssql = "SELECT * FROM `customerdetails` where bookingcode='$code' and status='ORDERED'";
    $result = mysqli_query($con, $ssql);
    $tot = 0;
    $slno = 0;
  ...
  <table class='table table-bordereless table-hover table-condensed small'>
    ...
    echo "<tr><th style='color:rebeccapurple;font-weight:bold' scope='row' colspan='13'> Total Number of order(quantity) placed on is " . $tot . "</th></tr>";

    echo '</table></div>';

    /*Inserting and displaying on code ends*/
  }
  /*displaying booked data with code ends*/


  /*displaying all  data starts*/
  $ssql = "SELECT * FROM `customerdetails` where status='ORDERED'";
  $result = mysqli_query($con, $ssql);
  $nrow=mysqli_num_rows($result);
  $tot = 0;
  $slno = 0;
  ...
  echo "<tr><th style='color:rebeccapurple;font-weight:bold' scope='row' colspan='13'>Number of order placed on  is " . $nrow . "</th></tr>";
  echo "<tr><th style='color:rebeccapurple;font-weight:bold' scope='row' colspan='13'> Total Number of order(quantity) placed is " . $tot . "</th></tr>";

  echo '</table></div>';

  /*displaying all data ends*/
